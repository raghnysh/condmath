...
\begin{exercise}
  ...
  Thus, the segments of \(X\) form a topology on \(X\) in which any
...
\begin{solution}[\ref{exe:k8tmok20}]
  ...
  (\Cref{exe:rpm18mof}).  For every segment \(\Sigma\) of \(\Lambda\),
  let \(F(\Sigma)\) be the set of strictly increasing functions from
  \(\Sigma\) to \(X\).  If \((\Sigma_i)_{i \in I}\) is a family of
  segments of \(\Lambda\), and \(\Sigma\) their union, then a function
  \(s : \Sigma \to X\) belongs to \(F(\Sigma)\) if and only if
  \(s \vert_{\Sigma_i}\) belongs to \(F(\Sigma_i)\) for every \(i\).
  Therefore, \(F\) is a subsheaf of the sheaf of \(X\)-valued
  functions on the topological space \(\Lambda\).  For every
