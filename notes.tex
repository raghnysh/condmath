...
\begin{theorem}
  \label{thm:4axgzui8}
  Let \(X\) be a well-ordered set.  Suppose that every element \(x\)
  of \(X\), \(F_x\) is an assignment that attaches to each family of
  sets \(E = (E_y)_{y \in (-\infty, x)}\) a set \(F_x(E)\).  Then
  there is a unique family of sets \(U = (U_x)_{x \in X}\) such that
  \(U_x = F_x((U_y)_{y \in (-\infty, x)})\) for all \(x \in X\).
\end{theorem}
...
\begin{theorem}[Principle of transfinite induction]
  \label{thm:a5jdoffh}
  Let \(x\) be a subset of an ordinal \(\alpha\) satisfying the
  following conditions:
  \begin{enumerate}
  \item If \(\beta\) is an element of \(\alpha\) such that \(\beta\)
    is a successor ordinal and \(\beta^- \in x\), then
    \(\beta \in x\).
  \item If \(\beta\) is an element of \(\alpha\) such that \(\beta\)
...
\Cref{thm:8odofa55}, \(\beta^- \in \beta\); as \(\beta\) is a subset
...
both the cases, \(\beta \in x\).  By \Cref{thm:6ehprsvw},
\(x = \alpha\).
...
\end{enumerate}
Then transfinite recursion (\Cref{thm:fa9kx7s9}) gives a unique
section \(u\) of \(p\) on \(\alpha\) such that
...
It is convenient to expand \Cref{thm:7oq31ljz} as follows.
...
theorem follows from \Cref{thm:7oq31ljz}.
...
This theorem can be deduced from \Cref{thm:7oq31ljz} just as
(\ref{thm:5kg5ewo1}) was deduced from (\ref{thm:srxrngd8}).
...
This theorem is an expansion of \Cref{thm:fi9epz7i} just as
...
\begin{theorem}[Principle of transfinite induction]
  ...
  true for every element \(\beta\) of \(\alpha\).  Then \(P(\alpha)\)
  is true for every ordinal \(\alpha\).
\end{theorem}
...
of \(x\).  Then \(P(\gamma)\) is true for every element \(\gamma\) of
\(\beta\).  Therefore, by the hypothesis on \(P\), \(P(\beta)\) is
true, so \(\beta \in x\).  By transfinite induction
